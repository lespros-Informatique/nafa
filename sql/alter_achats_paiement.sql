...
ALTER TABLE `achats`
  ...
  ADD COLUMN `statut_paiement_achat` enum('paye','partiel','credit') DEFAULT 'paye' AFTER `reste_a_payer_achat`,
...
WHERE `statut_paiement_achat` = 'paye' OR `statut_paiement_achat` IS NULL OR `statut_paiement_achat` = '';
